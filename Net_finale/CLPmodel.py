...
class Net(nn.Module):
    ...
    def forward(self, temp, x_k, mask):
        for i in range(self.num):
            ...
            #xd = fastmri.complex_abs(output)
            #fig = plt.figure()
            #plt.imshow(xd[0][0].detach().numpy(), cmap='gray')
            temp = fft_utils.ifft2((1.0 - mask) * fft_utils.fft2(output) + x_k) #DataConsistency Layer
        
        temp = T.complex_center_crop(temp, (320,320))
...
class ComplexConv2d(nn.Module):
    ...
    def forward(self, x):  # x.shape = (batch, channel, axis1, axis2, 2)
        ...
        #print("SCUSI SA L'ORA????, ",output.shape)
        return output
